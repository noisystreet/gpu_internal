...
- **跨厂商支持**：代码可在 NVIDIA、AMD、Intel、Apple 等任何支持 Vulkan 的 GPU 上运行，无需修改。这是 Vulkan 相比 CUDA 最核心的优势
- **极低的 API 开销**：Vulkan 的 CPU 端提交延迟约 1-5 us，是 CUDA 的 1/10 到 1/4，适合短生命周期、频繁启动的计算任务（如实时渲染中的计算后处理）
- **与图形渲染紧密集成**：Vulkan 天生支持在同一管线中混合图形和计算，计算着色器可以直接读写图形资源（帧缓冲、深度图等），无需上下文切换
- **精确的资源控制**：从内存分配到同步，所有操作由开发者显式控制。没有隐藏的线程同步或隐式内存拷贝，行为完全可预测

劣势
=======

- **开发效率低**：一个简单的向量加需要数百行样板代码（设备创建、管线编译、内存管理、屏障同步）。CUDA 中 5 行的工作，在 Vulkan 中需要 50 行
- **没有 CUDA 级别的数学库支持**：没有 cuBLAS、cuDNN、cuFFT 等成熟的高性能库。虽然可以用 GLSL 自己实现 GEMM，但优化难度极大，且缺乏 NVIDIA 那样深入的硬件优化
- **生态不如 CUDA 成熟**：AI 框架（PyTorch、TensorFlow）的 Vulkan 后端仍在早期阶段，多数框架主要以 CUDA 为目标平台。社区资源、教程和问题解决方案远少于 CUDA
- **调试工具不如 NVIDIA Nsight 完善**：Vulkan 依赖 Validation Layers 做错误检查（相比 CUDA 的 runtime 错误检查更早期但信息量更大），缺少类似 Nsight Compute 那样的 kernel 级性能剖析工具

**何时选择 Vulkan Compute**：

.. code-block:: text

   选择 Vulkan Compute 的场景:
   ✓ 需要跨厂商支持（不限于 NVIDIA）
   ✓ 计算与图形渲染紧密结合（游戏引擎、实时渲染）
   ✓ 极低延迟要求的短计算任务（每帧多次提交）
   ✓ 需要精确控制硬件资源（专业工具、框架开发）
